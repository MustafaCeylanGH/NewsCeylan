...
function TechnologyPage() {
  ...
  const [technologyArticle, setTechnologyArticle] = useState([]);

  useEffect(() => {
    const loadNews = async () => {
      setLoading(true);
      const news = await fetchNews("technology");
      setTechnologyArticle(news.articles);
      setLoading(false);
    };
    loadNews();
  }, [setLoading]);
    ...
    <div className={styles.mainContainer}>
      ...
      <div className={styles.newsContainer}>
        ...
        <div className={styles.leftUpperColumn}>
          {technologyArticle.slice(0, 1).map((article, index) => {
            return (
              <NewsCard
                key={index}
                title={article.title}
        ...
        <div className={styles.leftDownColumn}>
          {technologyArticle.slice(1, 4).map((article, index) => {
            return (
              <NewsCard
                key={index}
                title={article.title}
        ...
        <div className={styles.rightColumn}>
          {technologyArticle.slice(4, 6).map((article, index) => {
            return (
              <NewsCard
                key={index}
                title={article.title}
